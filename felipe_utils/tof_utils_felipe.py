...
from felipe_utils.research_utils.timer import Timer
...
def simulate_n_photons(transient, n_photons=1, n_mc_samples=1):
    ...
    for i in range(n_elems):
        ...
        simulated_transient[:, i, :] = rng.multinomial(n=n_photons,
                                                       pvals=curr_transient.astype(np.float64) / curr_transient.astype(
                                                           np.float64).sum(), size=n_mc_samples)
...
def hist2timestamps(hist_tensor, max_n_timestamps=None):
    ...
    return timestamp_tensor.reshape(hist_shape[0:-1] + (max_n_timestamps,)), n_timestamp_per_elem.reshape(
        hist_shape[0:-1])
...
def set_avg_power(v, pAveSource, num_measurements=1, ambient=None, sbr=None, T=0.1, mean_beta=1, dt=1, tau=1,
    ...
    v_out *= num_measurements
    return v_out
def calculate_ambient_v(v, pAveAmbient, dt, tau):
...
def set_sbr_pAveSource(v, pAveSource=None, sbr=None, dt=1, tau=1, axis=-1, inplace=False):
    ...
    if (not (sbr is None)):
        ...
        ambient = calculate_ambient_v(v, pAveAmbient, dt, tau)
        if (inplace):
            # do not create copy of vector
            v += ambient
            return v
