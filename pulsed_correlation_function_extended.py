...
if __name__ == "__main__":
    ...
    SAVE_NAME = f'corsemk{NUM_GATES}_{MHZ}mhz_{VOLTAGE}v_{DUTY}w_correlations'

    GATE_WIDTH = math.ceil((LASER_TAU // NUM_GATES) * 1e-3 )
    ...
    for i in range(NUM_GATES):
        ...
        while current_intTime > 480:
            ...
            correlations[:, :, i, :] += SPAD1.get_gated_intensity(BIT_DEPTH, 480, ITERATIONS, GATE_STEPS, GATE_STEP_SIZE,
                                                    GATE_STEP_ARBITRARY, GATE_WIDTH,
                                                    gate_start, GATE_DIRECTION, GATE_TRIG, OVERLAP, 1, PILEUP,
                                                    IM_WIDTH)
            current_intTime -= 480

        correlations[:, :, i, :]  += SPAD1.get_gated_intensity(BIT_DEPTH, current_intTime, ITERATIONS, GATE_STEPS, GATE_STEP_SIZE,
                                                GATE_STEP_ARBITRARY, GATE_WIDTH,
                                                gate_start, GATE_DIRECTION, GATE_TRIG, OVERLAP, 1, PILEUP,
                                                IM_WIDTH)
